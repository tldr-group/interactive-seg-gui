...
@author: FooBar167, stackoverflow
Adapted - have made some edits to set_image but otherwise will treat as a black-box.
...
# looks like image reloaded every time zoom is changed. can make a method to set the image rather than having to load from file
# show image is called whenever there's a KBM event.
...
class CanvasImage:
    ...
    def set_images(self, pil_image):
        self.__image = pil_image
        self.__original_image = pil_image

    def set_current_image(self, pil_image, new=False):
        if new:
            # Bind events to the Canvas
            self.canvas.bind("<Configure>", lambda event: self._show_image())  # canvas is resized
            self.canvas.bind("<Control_L>", self.__move_from)  # remember canvas position
            self.canvas.bind("<Control-Motion>", self.__move_to)  # move canvas to the new position
        ...
        if (
            self.imwidth * self.imheight > self.__huge_size * self.__huge_size and self.__image.tile[0][0] == "raw"
        ...
        self.__pyramid = [self.smaller()] if self.__huge else [self.__original_image]
    ...
    def smaller(self):
        ...
        while i < self.imheight:
            ...
            self.__image.close()
            self.__image = self.__original_image  # Image.open(self.path)  # reopen / reset image
    ...
    def redraw_figures(self):
        """Redraw figures in the children classes."""
        pass
    ...
    def __move_to(self, event):  # maybe track this and add the offset to the bbox?
...
class MainWindow(ttk.Frame):
    ...
    def __init__(self, mainframe):
        ...
        canvas = CanvasImage(self.master)  # create widget
        canvas.set_current_image(Image.open(filename), new=True)
